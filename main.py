...
# 葡萄牙语首页
@app.get("/pt")
async def serve_index_pt():
    """葡萄牙语首页 - Baixar Vídeos do TikTok"""
    return FileResponse(os.path.join(frontend_path, "pt", "index.html"))


@app.get("/pt/mp3")
async def serve_mp3_pt():
    """葡萄牙语MP3转换页面"""
    return FileResponse(os.path.join(frontend_path, "pt", "mp3.html"))


@app.get("/pt/miniatura")
async def serve_thumbnail_pt():
    """葡萄牙语封面图下载页面"""
    return FileResponse(os.path.join(frontend_path, "pt", "miniatura.html"))


@app.get("/pt/historia")
async def serve_story_pt():
    """葡萄牙语Story下载页面"""
    return FileResponse(os.path.join(frontend_path, "pt", "historia.html"))


# 葡萄牙语法律页面路由
@app.get("/pt/{page_name}")
async def serve_page_pt(page_name: str):
    """葡萄牙语法律/信息页面"""
    allowed_pages = ["contato", "privacidade", "termos", "aviso-legal", "dmca", "cookies"]
    if page_name not in allowed_pages:
    ...
    page_map = {
        ...
        "aviso-legal": "aviso-legal.html",   # Aviso Legal
        "dmca": "dmca.html",                 # DMCA
        "cookies": "cookies.html"            # Política de Cookies
    }
...
# 德语首页
@app.get("/de")
async def serve_index_de():
    """德语首页 - TikTok-Videos Herunterladen"""
    return FileResponse(os.path.join(frontend_path, "de", "index.html"))


@app.get("/de/mp3")
async def serve_mp3_de():
    """德语MP3转换页面"""
    return FileResponse(os.path.join(frontend_path, "de", "mp3.html"))


@app.get("/de/miniatur")
async def serve_thumbnail_de():
    """德语缩略图页面"""
    return FileResponse(os.path.join(frontend_path, "de", "miniatur.html"))


@app.get("/de/story")
async def serve_story_de():
    """德语Story页面"""
    return FileResponse(os.path.join(frontend_path, "de", "story.html"))


# 德语法律页面路由
@app.get("/de/{page_name}")
async def serve_page_de(page_name: str):
    """德语法律/信息页面"""
    allowed_pages = ["kontakt", "datenschutz", "nutzungsbedingungen", "haftungsausschluss", "dmca", "cookies"]
    if page_name not in allowed_pages:
    ...
    page_map = {
        ...
        "haftungsausschluss": "haftungsausschluss.html",     # Haftungsausschluss
        "dmca": "dmca.html",                                 # DMCA
        "cookies": "cookies.html"                            # Cookie-Richtlinie
    }
    ...
    return FileResponse(os.path.join(frontend_path, "de", "pages", filename))
...
# ============== 印尼语路由 ==============
@app.get("/id")
async def serve_index_id():
    """印尼语首页 - Download Video TikTok"""
    return FileResponse(os.path.join(frontend_path, "id", "index.html"))


@app.get("/id/mp3")
async def serve_mp3_id():
    """印尼语MP3转换页面"""
    return FileResponse(os.path.join(frontend_path, "id", "mp3.html"))


@app.get("/id/thumbnail")
async def serve_thumbnail_id():
    """印尼语缩略图页面"""
    return FileResponse(os.path.join(frontend_path, "id", "thumbnail.html"))


@app.get("/id/story")
async def serve_story_id():
    """印尼语Story页面"""
    return FileResponse(os.path.join(frontend_path, "id", "story.html"))


# 印尼语法律页面路由
@app.get("/id/{page_name}")
async def serve_page_id(page_name: str):
    """印尼语法律/信息页面"""
    allowed_pages = ["contact", "privacy", "terms", "disclaimer", "dmca", "cookies"]
    if page_name not in allowed_pages:
        raise HTTPException(status_code=404, detail="Page not found")
    
    file_path = os.path.join(frontend_path, "id", "pages", f"{page_name}.html")
